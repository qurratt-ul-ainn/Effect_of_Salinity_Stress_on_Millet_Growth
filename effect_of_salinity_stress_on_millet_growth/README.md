...
This experiment contains no geographic coordinates, field boundaries, soil sampling points, or raster layers. A scientifically valid map cannot be produced from this dataset alone. The repository therefore does **not invent a spatial map**. A GIS map can be added later if coordinates or an experimental layout are supplied.

## Reproducibility
The raw workbook is preserved unchanged. All derived data, tables, figures, and interpretations are generated by R scripts.
